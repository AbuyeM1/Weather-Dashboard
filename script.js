...
function searchWeather() {
    ...
    $.ajax({
        ...
        $("#cityName").html(response.name + "  (" + date + ")" + "  <img src=" + imageUrl2 + ">");
    ...
    $.ajax({
        ...
        for (var i = 0; i < 5; i++) {
            var date2 = new Date(response.list[((i + 1) * 8) - 1].dt * 1000).toLocaleDateString();

            $("#day" + (i + 1)).html(date2);
            var farTemp2 = ((response.list[((i + 1) * 8) - 1].main.temp - 273.15) * 1.8) + 32;
            $("#temp" + (i + 1)).html(farTemp2.toFixed(2));
            $("#humidity" + (i + 1)).html(response.list[((i + 1) * 8) - 1].main.humidity + "%");
